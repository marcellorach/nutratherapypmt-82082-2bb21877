-- Backfill pet_profiles for orphaned pet_ids referenced by approved (complete) clinical analysis snapshots.
-- These were generated by ephemeral demo pets; we materialize them so the tutor view can display the analyses.
...
numbered AS (
  SELECT pet_id, ROW_NUMBER() OVER (ORDER BY pet_id) AS rn
  FROM orphans
),
samples AS (
  SELECT * FROM (VALUES
