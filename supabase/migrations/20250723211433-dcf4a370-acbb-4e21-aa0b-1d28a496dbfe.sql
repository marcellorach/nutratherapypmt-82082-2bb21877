...
-- Corrigir nome do MSM e criar relações com o nome correto
-- Primeiro, vamos garantir que as condições existem
...
('Estresse oxidativo', 'Danos causados por radicais livres'),
('Fadiga', 'Cansaço e baixa energia'),
...
('Saúde cardiovascular', 'Saúde geral do sistema cardiovascular')
...
-- Criar relações para MSM (com nome correto)
INSERT INTO nutraceutical_conditions (
  nutraceutical_id, 
  condition_id,
  relationship_type,
...
FROM nutraceuticals n
CROSS JOIN health_conditions c
WHERE n.name = 'MSM (Metilsulfonilmetano)'
...
SELECT 
  ...
  CASE 
    ...
    WHEN n.name = 'MSM (Metilsulfonilmetano)' THEN 4.0
  ...
  CASE 
    ...
    WHEN n.name = 'MSM (Metilsulfonilmetano)' THEN 'Metilsulfonilmetano com evidências sólidas para saúde articular e redução da inflamação'
    WHEN n.name = 'Taurina' THEN 'Aminoácido com evidências robustas para saúde cardiovascular e função cardíaca'
  END as notes
FROM nutraceuticals n
WHERE n.name IN ('SAMe', 'MSM (Metilsulfonilmetano)', 'Taurina')
