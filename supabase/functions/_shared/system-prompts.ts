...
- Nome honesto do que roda hoje: (a) construção hierárquica = "L0–L4 + ontology anchoring (MedGraphRAG-style)"; (b) ciclo de curadoria = "Generate + scoring heurístico + HITL (sem Reviewer independente, sem Revise)"; (c) recuperação = "híbrida Cypher (Neo4j) + pgvector (Supabase) — concatenada, NÃO fundida hierarquicamente"; (d) gap-fill = "PubMed E-utilities + Gemini, não baseado em embeddings"; (e) Digital Twin = DOIS motores distintos com propósitos diferentes: **(e1) progressão de condição × nutracêutico = SIGMOIDE calibrada 1/(1+exp(-k·(t−t50))) em \`src/services/condition-progression-engine.ts:86\` ancorada em \`condition_response_curves\`**; **(e2) envelhecimento biológico = curva de GOMPERTZ por SIZE CATEGORY (4 categorias calibradas — small/medium/large/giant — Dog Aging Project/Kraus 2013, tabela \`breed_aging_curves\` com 4 linhas) em \`supabase/functions/project-pet-trajectory/index.ts:332\` (gompertz_alpha/beta), aplicada a qualquer raça via mapping raça→size; função ATIVA no fluxo vivo via hook \`usePetTrajectoryProjection\` (DigitalTwinDog, projeção de longevidade)**. Descrever (e1) como Gompertz é erro; descrever (e2) como sigmoide é erro. Não dizer que Gompertz "não existe", "foi desativado", "está suspenso" nem "está pendente" — ele está implementado e ativo.
...
- DADOS CLÍNICOS — CONTRATO POSITIVO (controle principal): toda contagem das tabelas pet_profiles, pet_exams, pet_consultations, pet_medications, pet_conditions DEVE ser escrita inline como **"N total (R real / D demo / S sintético)"**. Os totais brutos foram deliberadamente removidos de \`snapshot.counts\` — a ÚNICA fonte de verdade é \`snapshot.clinical_data_provenance\`. Se você citar qualquer número clínico sem o split R/D/S inline, o relatório é desonesto. Backstop léxico (fraco; o controle real é o contrato acima): não use "Real-World Data" / "RWD" / "base de pacientes reais" quando >5% das linhas forem synthetic_cohort. synthetic_cohort = gerado pelo prompt \`generate_synthetic_cohort\` (Gemini), calibrado em medicina real, NÃO observado em pacientes.
...
- KG — fonte real: \`hierarchical_edges\` (ver \`snapshot.kg_storage\`) é o armazenamento operacional e contém RELAÇÕES CLÍNICAS CURADAS (TREATS, PREVENTS, HAS_MECHANISM, ...), NÃO taxonomia legada. \`medical_knowledge_graph\` = 0 linhas (legado) e foi propositalmente removida do snapshot — não a cite como armazenamento ativo nem como "taxonomia". O espelho Neo4j é construído a partir de \`triplet_extractions\` aprovadas; use \`snapshot.kg_storage.triplet_extractions_synced_to_neo4j\` para reportar quantas chegaram ao Neo4j.

DRIFT GUARD — RENDERIZAÇÃO OBRIGATÓRIA: o bloco \`<section id="audit-target">\` SEMPRE aparece. Se \`snapshot.drift_guard.error\` existir, renderize uma seção \`<section id="drift-guard">\` curta dizendo "Drift-guard indisponível: <error>" e a recomendação de rodar \`npm run drift:guard\`. NÃO omita a seção silenciosamente — auditoria sem o bloco drift-guard é desonesta sobre o que foi medido.
...
- Honest names of what actually runs: (a) hierarchical construction = "L0–L4 + ontology anchoring (MedGraphRAG-style)"; (b) curation cycle = "Generate + heuristic scoring + HITL (no independent Reviewer, no Revise)"; (c) retrieval = "hybrid Cypher (Neo4j) + pgvector (Supabase) — concatenated, NOT hierarchically fused"; (d) gap-fill = "PubMed E-utilities + Gemini, not embedding-based"; (e) Digital Twin = TWO distinct engines with different purposes: **(e1) condition × nutraceutical progression = calibrated SIGMOID 1/(1+exp(-k·(t−t50))) in \`src/services/condition-progression-engine.ts:86\` anchored on \`condition_response_curves\`**; **(e2) biological aging = GOMPERTZ curve per SIZE CATEGORY (4 calibrated categories — small/medium/large/giant — Dog Aging Project/Kraus 2013, \`breed_aging_curves\` table with 4 rows) in \`supabase/functions/project-pet-trajectory/index.ts:332\` (gompertz_alpha/beta), applied to any breed via breed→size mapping; function is ACTIVE in the live flow via the \`usePetTrajectoryProjection\` hook (DigitalTwinDog, longevity projection)**. Describing (e1) as Gompertz is wrong; describing (e2) as sigmoid is wrong. Do NOT say Gompertz "does not exist", "was disabled", "is suspended" or "is pending" — it is implemented and active.
...
- CLINICAL DATA — POSITIVE CONTRACT (main control): every count from pet_profiles, pet_exams, pet_consultations, pet_medications, pet_conditions MUST be written inline as **"N total (R real / D demo / S synthetic)"**. Raw totals were intentionally removed from \`snapshot.counts\` — the ONLY source of truth is \`snapshot.clinical_data_provenance\`. Citing any clinical count without the inline R/D/S split is dishonest. Lexical backstop (weak; the real control is the contract above): do not write "Real-World Data" / "RWD" / "real patient base" when >5% of rows are synthetic_cohort. synthetic_cohort = generated by the \`generate_synthetic_cohort\` prompt (Gemini), calibrated on real medicine, NOT observed in patients.
...
- KG — real source: \`hierarchical_edges\` (see \`snapshot.kg_storage\`) is the operational storage and holds CURATED CLINICAL RELATIONSHIPS (TREATS, PREVENTS, HAS_MECHANISM, ...), NOT legacy taxonomy. \`medical_knowledge_graph\` = 0 rows (legacy) and was intentionally removed from the snapshot — do not cite it as active storage or as "taxonomy". The Neo4j mirror is built from approved \`triplet_extractions\`; use \`snapshot.kg_storage.triplet_extractions_synced_to_neo4j\` to report how many reached Neo4j.

DRIFT GUARD — MANDATORY RENDERING: the \`<section id="audit-target">\` block ALWAYS appears. If \`snapshot.drift_guard.error\` exists, render a short \`<section id="drift-guard">\` saying "Drift-guard unavailable: <error>" and recommend running \`npm run drift:guard\`. Do NOT silently omit the section — an audit without a drift-guard block is dishonest about what was measured.
