...
- Nome honesto do que roda hoje: (a) construção hierárquica = "L0–L4 + ontology anchoring (MedGraphRAG-style)"; (b) ciclo de curadoria = "Generate + scoring heurístico + HITL (sem Reviewer independente, sem Revise)"; (c) recuperação = "híbrida Cypher (Neo4j) + pgvector (Supabase) — concatenada, NÃO fundida hierarquicamente"; (d) gap-fill = "PubMed E-utilities + Gemini, não baseado em embeddings"; (e) Digital Twin = sigmoide calibrada 1/(1+exp(-k·(t−t50))) em \`src/services/condition-progression-engine.ts\` ancorada em \`condition_response_curves\`. **Gompertz NÃO está implementado em lugar nenhum do código. NÃO existe \`breed_aging_curves\`, NÃO existe \`project-pet-trajectory\` com curva de Gompertz, NÃO existe motor por raça. Toda menção a "Gompertz" ou "curva de envelhecimento por raça" como recurso do Senex é proibida.**
...
- Honest names of what actually runs: (a) hierarchical construction = "L0–L4 + ontology anchoring (MedGraphRAG-style)"; (b) curation cycle = "Generate + heuristic scoring + HITL (no independent Reviewer, no Revise)"; (c) retrieval = "hybrid Cypher (Neo4j) + pgvector (Supabase) — concatenated, NOT hierarchically fused"; (d) gap-fill = "PubMed E-utilities + Gemini, not embedding-based"; (e) Digital Twin = calibrated sigmoid 1/(1+exp(-k·(t−t50))) in \`src/services/condition-progression-engine.ts\` anchored on \`condition_response_curves\`. **Gompertz is NOT implemented anywhere in the codebase. There is NO \`breed_aging_curves\`, NO Gompertz-based \`project-pet-trajectory\`, NO per-breed aging engine. Any mention of "Gompertz" or "per-breed aging curve" as a Senex feature is forbidden.**
