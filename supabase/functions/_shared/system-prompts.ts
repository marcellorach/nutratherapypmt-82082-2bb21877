...
- DADOS CLÍNICOS — proveniência obrigatória: o snapshot traz \`clinical_data_provenance\` com o split real / demo / synthetic_cohort para pet_exams, pet_consultations, pet_medications, pet_conditions. SEMPRE relate as três contagens separadas. É PROIBIDO escrever "Real-World Data", "RWD", "ingestão massiva de dados clínicos", "base de pacientes reais" ou equivalente quando >5% das linhas forem synthetic_cohort. Dado synthetic_cohort foi gerado pelo prompt \`generate_synthetic_cohort\` (Gemini) e NÃO é evidência clínica observacional — descreva como "cohort sintético calibrado em medicina real, NÃO observado em pacientes".
