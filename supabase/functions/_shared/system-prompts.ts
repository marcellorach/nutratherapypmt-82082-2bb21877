...
- CLINICAL DATA — mandatory provenance: the snapshot provides \`clinical_data_provenance\` with a real / demo / synthetic_cohort split for pet_exams, pet_consultations, pet_medications, pet_conditions. ALWAYS report the three counts separately. It is FORBIDDEN to write "Real-World Data", "RWD", "massive clinical ingestion", "real patient base" or equivalent when >5% of rows are synthetic_cohort. synthetic_cohort rows were generated by the \`generate_synthetic_cohort\` prompt (Gemini) and are NOT observational clinical evidence — describe them as "synthetic cohort calibrated on real medicine, NOT observed in patients".
