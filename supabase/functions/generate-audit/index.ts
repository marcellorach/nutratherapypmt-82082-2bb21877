...
const FALLBACK_COVERAGE: CoverageItem[] = [
  ...
  { id: "ai-scientist-predictive-models", pillar: "AI Scientist", title_pt: "Catálogo de modelos preditivos ancorados em cohorts (confirmar quantidade pelo snapshot — sem chutar número)", expected: "active", evidence: "predictiveModelsData.ts" },
  ...
  { id: "breed-predispositions", pillar: "Análise do Paciente & Recomendação Híbrida", title_pt: "Predisposições por raça (contagem real de raças e predisposições deve vir do snapshot — não use 81/254 nem qualquer número sem origem)", expected: "active", evidence: "breeds + breed_predispositions" },
  ...
  { id: "digital-twin", pillar: "Digital Twin & Projeções", title_pt: "Digital Twin & projeções — DOIS motores distintos com propósitos diferentes: (a) progressão de condição × nutracêutico = SIGMOIDE calibrada 1/(1+exp(-k·(t−t50))) em condition-progression-engine.ts:86, ancorada em condition_response_curves; (b) envelhecimento biológico = GOMPERTZ por SIZE CATEGORY (4 categorias calibradas — small/medium/large/giant — Dog Aging Project/Kraus 2013, breed_aging_curves com 4 linhas) em project-pet-trajectory/index.ts:332, aplicada a qualquer raça via mapping raça→size; função ATIVA via hook usePetTrajectoryProjection. NÃO descreva (a) como Gompertz; NÃO diga que Gompertz não existe / foi desativado / está suspenso.", expected: "active", evidence: "condition-progression-engine.ts:86 (sigmoide) + condition_response_curves + breed_aging_curves (4 linhas, por size) + project-pet-trajectory/index.ts:332 (Gompertz) + usePetTrajectoryProjection (consumidor vivo)" },
...
async function readAuditContext(service: ReturnType<typeof createClient>, appOrigin?: string) {
  ...
  // NOTE: clinical-population tables (pet_profiles, pet_exams, pet_consultations,
  // pet_medications, pet_conditions) are intentionally NOT in `tableNames`.
  // Their counts are exposed ONLY via `clinical_data_provenance` below as
  // { real, demo, synthetic_cohort } — never as a raw total — to prevent the
  // LLM from describing synthetic_cohort rows as "real activity".
  // `medical_knowledge_graph` is also excluded (legacy, 0 rows). The real
  // graph storage is `hierarchical_edges` (broken down by relationship below).
  const tableNames = [
    "processed_studies", "study_embeddings", "triplet_extractions",
    "hierarchical_edges", "breeds",
  ...
  snapshot.counts_note = "Clinical-population tables (pet_profiles, pet_exams, pet_consultations, pet_medications, pet_conditions) are NOT in `counts` on purpose — see `clinical_data_provenance` for the {real, demo, synthetic_cohort} split. `medical_knowledge_graph` is excluded (legacy, 0 rows); the operational KG storage is `hierarchical_edges` (see `kg_storage` below).";

  // KG storage breakdown — exposes that `hierarchical_edges` holds 38k+
  // curated CLINICAL relationships (TREATS/PREVENTS/HAS_MECHANISM/...),
  // not legacy taxonomy. Also reports how many approved triplets reached Neo4j.
  try {
    ...
    const topRelationships = Object.entries(byRel)
    ...
    const { count: syncedCount } = await service
      .from("triplet_extractions")
      .select("*", { count: "exact", head: true })
      .eq("curation_status", "approved")
      .eq("synced_to_neo4j", true);
    snapshot.kg_storage = {
      note: "`hierarchical_edges` is the operational KG storage (Supabase). It holds CURATED CLINICAL relationships (TREATS/PREVENTS/HAS_MECHANISM/...), NOT taxonomy. `medical_knowledge_graph` is legacy (0 rows). Neo4j mirror is built from approved triplet_extractions; `synced_to_neo4j` shows how many actually reached the graph DB.",
      hierarchical_edges_top_relationships: topRelationships,
      triplet_extractions_approved: approvedCount ?? 0,
      triplet_extractions_synced_to_neo4j: syncedCount ?? 0,
    };
  } catch (e) {
  ...
  try {
    const { data: petRows } = await service
      .from("pet_profiles")
      .select("id,is_demo,is_synthetic");
    ...
    const split = async (table: string) => {
      const { data } = await service.from(table).select("pet_id");
    ...
    snapshot.clinical_data_provenance = {
      note: "THIS IS THE ONLY SOURCE OF TRUTH for clinical-population counts. The raw totals are intentionally not exposed elsewhere in the snapshot. synthetic_cohort = generated by generate-synthetic-cohort prompt (Gemini) — NOT observed in real patients, NOT Real-World Data. Every count cited in the report MUST be written inline as 'N total (R real / D demo / S sintético)'.",
...
{{PRIOR_AUDITS}}`;
  ...
  const ROUND_EMPHASIS_PT = `\n\nÊNFASES DESTA RODADA (correções de honestidade — aplicar antes de qualquer outra regra):\n1) DIGITAL TWIN — DOIS motores distintos, propósitos diferentes, ambos ATIVOS: (a) progressão de condição × nutracêutico = SIGMOIDE calibrada 1/(1+exp(-k·(t−t50))) em condition-progression-engine.ts:86, ancorada em condition_response_curves; (b) envelhecimento biológico = GOMPERTZ por SIZE CATEGORY (4 categorias calibradas — small/medium/large/giant — Dog Aging Project/Kraus 2013, tabela breed_aging_curves com 4 linhas) em supabase/functions/project-pet-trajectory/index.ts:332 (gompertz_alpha/beta), aplicada a qualquer raça via mapping raça→size, ATIVA via hook usePetTrajectoryProjection. Descrever (a) como Gompertz é erro doc. Dizer que Gompertz "não existe", "foi desativado", "está suspenso", "downgrade" ou "pendente" é ERRO — ele está implementado e ativo no propósito (b).\n2) DADOS CLÍNICOS — relate SEMPRE o split real / demo / synthetic_cohort do snapshot (\`clinical_data_provenance\`). Toda contagem clínica DEVE ser escrita inline como "N total (R real / D demo / S synthetic)". Backstop léxico fraco: não escreva "RWD", "dados do mundo real", "base de pacientes reais" quando >5% das linhas é synthetic_cohort.\n3) SEM CRÉDITO EMPRESTADO — GRRA, U-Retrieval (top-down/bottom-up) e TransE são INSPIRAÇÃO científica, não mecanismos do Senex. Se citá-los, marque "(inspiração; não implementado)".\n`;
  const ROUND_EMPHASIS_EN = `\n\nROUND EMPHASES (honesty corrections — apply before any other rule):\n1) DIGITAL TWIN — TWO distinct engines, different purposes, both ACTIVE: (a) condition × nutraceutical progression = calibrated SIGMOID 1/(1+exp(-k·(t−t50))) in condition-progression-engine.ts:86, anchored on condition_response_curves; (b) biological aging = GOMPERTZ per SIZE CATEGORY (4 calibrated categories — small/medium/large/giant — Dog Aging Project/Kraus 2013, breed_aging_curves table with 4 rows) in supabase/functions/project-pet-trajectory/index.ts:332 (gompertz_alpha/beta), applied to any breed via breed→size mapping, ACTIVE via the usePetTrajectoryProjection hook. Describing (a) as Gompertz is a doc bug. Saying Gompertz "does not exist", "was disabled", "is suspended", "downgraded" or "pending" is WRONG — it is implemented and active for purpose (b).\n2) CLINICAL DATA — ALWAYS report the real / demo / synthetic_cohort split from the snapshot (\`clinical_data_provenance\`). Every clinical count MUST be written inline as "N total (R real / D demo / S synthetic)". Weak lexical backstop: do not write "RWD", "real-world data", "real patient base" when >5% of rows are synthetic_cohort.\n3) NO BORROWED CREDIT — GRRA, U-Retrieval (top-down/bottom-up) and TransE are scientific INSPIRATION, not Senex mechanisms. If named, tag them "(inspiration; not implemented)".\n`;
  const emphasis = lang === "en" ? ROUND_EMPHASIS_EN : ROUND_EMPHASIS_PT;
