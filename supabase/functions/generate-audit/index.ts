...
{{PRIOR_AUDITS}}`;
  ...
  const ROUND_EMPHASIS_PT = `\n\nÊNFASES DESTA RODADA (correções de honestidade — aplicar antes de qualquer outra regra):\n1) DIGITAL TWIN — o motor de progressão de condição com nutracêutico é SIGMOIDE (1/(1+exp(-k·(t−t50))), condition-progression-engine.ts:86). Qualquer menção a "Gompertz" como motor de resposta a tratamento é ERRO DE DOCUMENTAÇÃO — REPORTE como erro/inconsistência doc, NÃO descreva Gompertz como recurso implementado nesse contexto. Gompertz só aparece honestamente em breed_aging_curves (envelhecimento biológico por raça, função separada).\n2) DADOS CLÍNICOS — relate SEMPRE o split real / demo / synthetic_cohort do snapshot (\`clinical_data_provenance\`). PROIBIDO escrever "RWD", "dados do mundo real", "base de pacientes reais", "ingestão massiva de dados clínicos" (≈98% das linhas é synthetic_cohort gerado pelo prompt generate_synthetic_cohort). Os números "81 raças", "254 predisposições", "6 modelos preditivos" só podem aparecer se vierem do snapshot — caso contrário marque "n/d (não confirmado pelo snapshot)".\n3) SEM CRÉDITO EMPRESTADO — GRRA, U-Retrieval (top-down/bottom-up) e TransE são INSPIRAÇÃO científica, não mecanismos do Senex. Se citá-los, marque "(inspiração; não implementado)" e descreva o que efetivamente roda (heurística + HITL; Cypher + pgvector concatenados; PubMed E-utilities + Gemini).\n`;
  const ROUND_EMPHASIS_EN = `\n\nROUND EMPHASES (honesty corrections — apply before any other rule):\n1) DIGITAL TWIN — the condition-with-nutraceutical progression engine is SIGMOID (1/(1+exp(-k·(t−t50))), condition-progression-engine.ts:86). Any mention of "Gompertz" as the treatment-response engine is a DOCUMENTATION BUG — REPORT it as a doc inconsistency, do NOT describe Gompertz as an implemented feature in that context. Gompertz only appears honestly in breed_aging_curves (per-breed biological aging, separate function).\n2) CLINICAL DATA — ALWAYS report the real / demo / synthetic_cohort split from the snapshot (\`clinical_data_provenance\`). FORBIDDEN to write "RWD", "real-world data", "real patient base", "massive clinical ingestion" (~98% of rows is synthetic_cohort produced by the generate_synthetic_cohort prompt). The figures "81 breeds", "254 predispositions", "6 predictive models" may appear ONLY if present in the snapshot — otherwise mark "n/a (not confirmed by snapshot)".\n3) NO BORROWED CREDIT — GRRA, U-Retrieval (top-down/bottom-up) and TransE are scientific INSPIRATION, not Senex mechanisms. If named, tag them "(inspiration; not implemented)" and describe what actually runs (heuristic + HITL; Cypher + pgvector concatenated; PubMed E-utilities + Gemini).\n`;
