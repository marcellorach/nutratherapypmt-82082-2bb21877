...
const ModelosPreditivosTab = () => {
  ...
  const translatedModels = useTranslatedPredictiveModels();
  ...
  const filteredModelos = translatedModels.filter((modelo) =>
  ...
  // Get all insights from the first model (Efficacy Prediction model) - Already translated
  const recentInsights = translatedModels[0]?.degenerativeInsights || [];
      ...
      <Card className="p-6 border-border bg-card">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-foreground">
              {t('predictiveModels.table.title')}
            </h3>
            <p className="text-sm text-muted-foreground">
              {t('predictiveModels.table.subtitle')}
            </p>
          </div>
          <div className="relative w-64">
            ...
            <Input
              type="text"
              placeholder={t('predictiveModels.search')}
              value={searchTerm}
            ...
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">
                  {t('predictiveModels.table.model')}
                </th>
                <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">
                  {t('predictiveModels.algorithm')}
                </th>
                <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">
                  {t('predictiveModels.accuracy')}
                </th>
                <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">
                  {t('predictiveModels.table.petsMonitored')}
                </th>
                <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">
                  {t('predictiveModels.table.monthlyLearning')}
                </th>
                <th className="text-right py-3 px-4 text-sm font-medium text-muted-foreground">
                  {t('predictiveModels.actions')}
                </th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={modelo.modelId} className="border-b border-border hover:bg-muted/30 transition-colors">
                          ...
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {hasRecentInsight(modelo) && (
                              <div className="flex items-center gap-1.5 text-muted-foreground">
                                ...
                                <span className="text-xs">{t('predictiveModels.table.recentData')}</span>
                              </div>
                            )}
                          </div>
                        ...
                        <div className="flex-1 min-w-[140px]">
                          ...
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            ...
                            <span className="text-xs">
                              {t('predictiveModels.table.lastTraining')}: {new Date(modelo.trainedAt).toLocaleDateString('pt-BR')}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="py-4 px-4">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {t('predictiveModels.table.treatment')}: {modelo.treatmentGroup.toLocaleString()} | {t('predictiveModels.table.control')}: {modelo.controlGroup.toLocaleString()}
                        </p>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                    ...
                    <td className="py-4 px-4 text-right">
                      ...
                      >
                        ...
                        {t('predictiveModels.table.viewEvolution')}
                      </Button>
                    </td>
                  </tr>
                );
        ...
        {filteredModelos.length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground">{t('predictiveModels.table.noModels')}</p>
          </div>
        )}
      </Card>
