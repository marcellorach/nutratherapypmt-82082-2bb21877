...
const ApiKeyForm: React.FC<ApiKeyFormProps> = ({ 
  ...
  const apiKeySchema = z.object({
    apiKey: z.string().min(minLength, t('apiKeyForm.validationMinLength', { min: minLength }))
  });
  ...
  const onSubmit = async (values: z.infer<typeof apiKeySchema>) => {
    try {
      ...
      toast({
        title: t('apiKeyForm.keySaved'),
        description: t('apiKeyForm.keySavedDesc', { service: serviceName }),
      });
    } catch (error) {
      toast({
        variant: "destructive",
        title: t('apiKeyForm.keySaveError'),
        description: t('apiKeyForm.keySaveErrorDesc', { error: error.message }),
      });
    }
  };
  ...
  const getValidationColor = () => {
    ...
    switch (validationResult.status) {
      case 'valid': return 'border-green-500 focus-visible:ring-green-500';
      case 'invalid': return 'border-red-500 focus-visible:ring-red-500';
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('apiKeyForm.apiKeyLabel', { service: serviceName })}</FormLabel>
              <FormControl>
